...
vec4 shading(vec4 color, vec4 light, vec3 material, vec3 worldPos, vec3 normal)
{
	...
	float blWhite = max(light.w, step(0.93625, light.x));
	...
	baseLight += albedo * light.w * EMISSIVE_LIGHT_STR;
	...
	vec3 skyLight = frx_worldHasSkylight * light.z * atmos_hdrCelestialRadiance() * (1. - frx_rainGradient);
