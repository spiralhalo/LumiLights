...
/*******************************************************
 *  lumi:shaders/pipeline/post/refraction.frag         *
 *******************************************************
...
{
    float depth = texture2DLod(u_translucent_depth, uv, 0).r;
...
{
    float depth = texture2DLod(u_solid_depth, uv, 0).r;
	vec3 clip = vec3(2.0 * uv - 1.0, 2.0 * depth - 1.0);
	vec4 view = inv_projection * vec4(clip, 1.0);
	return view.xyz / view.w;
}
...
{
	return frx_normalModelMatrix() * (2.0 * texture2DLod(u_normal_translucent, uv, 0).xyz - 1.0);
}

vec3 coords_normal(vec2 uv)
{
	return frx_normalModelMatrix() * (2.0 * texture2DLod(u_normal_solid, uv, 0).xyz - 1.0);
}
...
void main()
{
    float solid_depth = texture2DLod(u_solid_depth, v_texcoord, 0).r;
    float translucent_depth = texture2DLod(u_translucent_depth, v_texcoord, 0).r;
    float sky_light = texture2DLod(u_light_translucent, v_texcoord, 0).z;
