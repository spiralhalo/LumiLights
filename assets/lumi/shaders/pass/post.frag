...
void main()
{
	...
	if (notEndPortal(u_gbuffer_lightnormal) || albedo.a == 0.0) {
		fragColor += reflection(albedo.rgb, u_color_result, u_gbuffer_main_etc, u_gbuffer_lightnormal, u_translucent_depth, u_gbuffer_shadow, u_tex_nature, u_light_data, u_resources, idLight, idMaterial, idNormal, idMicroNormal);
	}
