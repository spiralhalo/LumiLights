...
vec3 hdr_calcSkyAmbientLight(inout light_data data)
{
    if (frx_worldHasSkylight())
    {
        ...
        vec3 skyRadiance = mix(0.1 * skyAmbientRadiance, l2_skyRadiance(data.light.y, frx_worldTime(), frx_ambientIntensity(), frx_rainGradient()), l2_clampScale(-0.1, 0.1, data.normal.y));
